...
input_path2 <- "data/intermediate/shared_table1.rds"
gen <- readRDS(file = input_path1)
meta <- readRDS(file = input_path2)

#structure runs datasets
str_files <- dir(path = "data/final", pattern = "run_shared")
...
lapply(function(x){
  ...
  subset_gen$other$metadata %>%
    ...
    as.data.frame %>%
    dplyr::select(ID) %>%
    write.table(file = file.path("data/intermediate/shared_clumpak",
