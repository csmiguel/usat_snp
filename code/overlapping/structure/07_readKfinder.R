...
kfinder <-
  dir("data/final", pattern = "str.K$", full.names = T, recursive = T) %>%
  grep(pattern = "shared", value = T)
...
# read runs
runs <- dir("data/final", pattern = "run_shared", full.names = T)
...
h <-
  seq_along(runs) %>%
    ...
    zz <- starmie::bestK(hh)
    # save plots
    ggplot2::ggsave(file.path(runs[x], "shared_evanno.pdf"), device = "pdf")
...
#save objects
saveRDS(kfinder_ls, "data/intermediate/shared_kfinder_ls.rds")
saveRDS(kfinder_tidy, "data/intermediate/shared_kfinder_tidy.rds")
saveRDS(best_files, "data/intermediate/shared_best_files_kfinder.rds")
saveRDS(h, "data/intermediate/shared_evanno.rds")
saveRDS(evanno_tidy, "data/intermediate/shared_evanno_tidy.rds")
